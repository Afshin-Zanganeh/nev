...
export class DataManager {
    ...
    public updateTreeDataStructure(node: TableNodeData, ownId: number[] = []) {
        ...
        if (node.isRootNode){
            const rule = node.getChildren()[0] as RuleNodeData;
            if(rule !== undefined) node.parameterPredicate = rule.ruleId.relevantHeadPredicate.parameters; //set the parameterPredicate of the root node
        }
    ...
    public resetFlagOnAllNodes(rootNode: TableNodeData, flag: "isGreyed") {
    ...
    //blurr delete Above
    public setFlagUntilThisNode(rootNode: TableNodeData, node: TreeNodeData, flag: "isGreyed") {
        const parents: TreeNodeData[] = [rootNode];
        for (const parent of parents) {
            if (parent === node) {
